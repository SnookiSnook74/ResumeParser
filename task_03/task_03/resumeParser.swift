...
class ResumeParser {
    ...
    func fullParseInfo() throws -> (CandidateInfo?, [Education], [WorkExperience], About?)? {
        guard let contents = readFile() else {
            print("Не удалось прочитать файл.")
            throw ResumeParserError.missingCandidateInfo
        }

        let candidateInfo = try parseCandidateInfo(from: contents)
    ...
    private func parseCandidateInfo(from contents: String) throws -> CandidateInfo? {
        guard contents.contains("# Candidate info") else {
            throw ResumeParserError.missingCandidateInfo
        }
