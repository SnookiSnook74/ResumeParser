...
do {
    if let (candidateInfo, _, workExp, aboutPerson) = try resumeParser.fullParseInfo() {
        ...
        ExportInfo.analyzeAndExportWordFrequency(from: aboutPerson?.text ?? "", level: careerLevel, profession: careerProfession, companies: companyGroup, matchingTagsPath: tags, to: exportFilePathWords)
    } else {
